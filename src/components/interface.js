...
These are the {list.length} matched units ({Math.round(100*list.length/total)}% of all {total}). Click a headline to sort on that aspect. Attack arrows that allow movement are shown in <span className='move'>red</span> in the grid.
                </p>
	        	<List units={list} /></div> }
	        </div>
        );
